...
void build_interface(AppData *app) {
    ...
    gtk_widget_set_margin_start(control_box, 4);
    gtk_widget_set_margin_end(control_box, 4);
    gtk_widget_set_margin_top(control_box, 4);
    gtk_widget_set_margin_bottom(control_box, 4);
    ...
    app->dc_source_button = gtk_button_new_with_label("Configure DC");
    gtk_box_append(GTK_BOX(control_box), app->dc_source_button);

    // Frequency-domain analysis button
    app->analysis_button = gtk_button_new_with_label("Freq/Small-Sig Analysis");
    gtk_box_append(GTK_BOX(control_box), app->analysis_button);
