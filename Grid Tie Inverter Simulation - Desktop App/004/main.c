...
static void on_start_button_toggled(GtkToggleButton *button, gpointer user_data) {
    ...
    if (app->params.running && app->timeout_id == 0) {
        app->timeout_id = g_timeout_add(16, simulation_update, app);
...
static void on_reset_button_clicked(GtkButton *button, gpointer user_data) {
    ...
    gtk_range_set_value(GTK_RANGE(app->timestep_scale), app->params.max_dt * 1000.0);
    ...
    if (app->analysis_window) {
        gtk_drop_down_set_selected(GTK_DROP_DOWN(app->analysis_type_dropdown), app->params.analysis_type);
...
static void on_scale_changed(GtkRange *range, gpointer user_data) {
    ...
    app->params.max_dt = gtk_range_get_value(GTK_RANGE(app->timestep_scale)) / 1000.0;
...
static void on_islanding_toggled(GtkWidget *switch_widget, gboolean state, gpointer user_data) {
    AppData *app = (AppData *)user_data;
...
static void on_analysis_button_clicked(GtkButton *button, gpointer user_data) {
    AppData *app = (AppData *)user_data;
    if (!app->analysis_window) {
        analysis_window_create(app);
    }
    gtk_window_present(GTK_WINDOW(app->analysis_window));
}
...
static void activate(GtkApplication *gtk_app, gpointer user_data) {
    ...
    g_signal_connect(app->voltage_scale, "value-changed", G_CALLBACK(on_scale_changed), app);
    g_signal_connect(app->frequency_scale, "value-changed", G_CALLBACK(on_scale_changed), app);
    g_signal_connect(app->phase_scale, "value-changed", G_CALLBACK(on_scale_changed), app);
    g_signal_connect(app->timestep_scale, "value-changed", G_CALLBACK(on_scale_changed), app);
    ...
    g_signal_connect(app->analysis_button, "clicked", G_CALLBACK(on_analysis_button_clicked), app);
